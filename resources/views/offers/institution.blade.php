...
<div class="block">
  <div class="row">
    <div class="col-md-12 col-sm-12">
      ...
      <div class="f-container f-align-baseline">
        <div class="f-grow-3">
          <ol class="breadcrumb">
      ...
      <div class="row">
        <div class="block-list-item">
          <div class="col-md-12 offer">
            ...
            <i class="info" teacher='{{ json_encode($offer->teachers) }}' classroom="{{ $offer->classroom }}" maxlessons="{{ $offer->maxlessons }}" day_period="{{ $offer->day_period }}"></i>

            <ul class="list-unstyled">
              @foreach ($offer->units as $unit)
              <li>
                <input {{ !strcmp($unit->status, 'E')? "checked" : "" }} class="toggle-btn toggle-event" type="checkbox"
                  data-toggle="toggle" data-on="Bloquear" data-off="Desbloquear" data-size="mini"
                  unit="{{ encrypt($unit->id) }}">
                &nbsp;&nbsp;
                <a class="text-link" target="_blank"
                  href='{{ URL::to("/classes/units/report-unit/".encrypt($unit->id)) }}'> Unidade {{ $unit->value }}</a>
              </li>
              @endforeach
            </ul>
            ...
            <div class="row">
              <div class="col-xs-12">
                <div class="list-inline">
                  <button class="btn btn-default btn-block-xs new-unit"
                    url='{{ URL::to("/classes/offers/unit/".encrypt($offer->id)) }}'> + Nova Unidade</button>
                  @if (!$offer->units->isEmpty())
                  <button class="btn btn-default btn-block-xs delete-unit"
