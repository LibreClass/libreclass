$(function () {
  ...
  $("#target").keypress(function (event) {
    if (event.which == 13) $("#find-teacher input[name='current']").val("0");
  });
  ...
  $("#new-teacher").click(function () {
    ...
    $("#modalAddTeacher input[name='enrollment']")
      .prop("disabled", false)
      .val("");
    $("#modalAddTeacher input[name='name']").prop("disabled", false).val("");
    $("#modalAddTeacher select[name='formation']")
      .prop("disabled", false)
      .val("");
  ...
  $("#modal-body", "#modalAddTeacherDiscipline").load(
    "/bind/list",
    {
    ...
    function () {
      ...
      $("#modalAddTeacherDiscipline #search").keyup(function () {
        ...
        for (i = 0; i < $(checkbox).length; i++) {
          ...
          if (text.indexOf(search) != -1) $(checkbox).eq(i).show();
          else $(checkbox).eq(i).hide();
        }
      });
    }
  );
  ...
  $("#modalAddTeacher input[name='email']").keyup(
    function () {
  ...
  function searchEmail(str) {
    ...
    $.post("/user/search-teacher", { str: str }, function (data) {
      if (data.status == 1) {
        $("#modalAddTeacher input[name='name']")
          .val(data.teacher.name)
          .prop("disabled", "disabled");
        $("#modalAddTeacher select[name='formation']")
          .val(data.teacher.formation)
          .prop("disabled", "disabled");
        $("#modalAddTeacher input[name='teacher']").val(data.teacher.id);
        $("#modalAddTeacher input[name='registered']").val("true");
        $("#modalAddTeacher .teacher-message")
...
$(".spinner-enrrollment").hide();
  $("#search input[name='enrollment']").keyup(function () {
    $(".spinner-enrrollment").show();
    clearTimeout(timeout);
    timeout = setTimeout(
      function () {
        searchEnrrolment($("#search input[name='enrollment']").val());
      }.bind(this),
      1000
    );
  }.bind(this)
